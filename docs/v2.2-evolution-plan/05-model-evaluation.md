...
| 7 | BGM 生成 | N20 | **ACE-Step 1.5** | Stable Audio 2.5 / 曲库 | <4GB / ~10GB | ComfyUI-ACE-Step | MIT / StabilityAI |
...
| 场景 | 模型 | HuggingFace 路径 | VRAM (全精度) | 说明 |
|------|------|------------------|-------------|------|
| **角色固化 (N09)** | FireRed-1.1 | `xiaoyuan-hb/FireRed-V1.1` | ~24GB BF16 | MultiRef 固化全套变体 |
| **关键帧一致性 (N10/N13)** | FireRed-1.1 + FLUX | — | ~24GB | FireRed MultiRef + FLUX 出图 |
...
```bash
# FireRed-1.1
huggingface-cli download xiaoyuan-hb/FireRed-V1.1 \
...
## 5. BGM 背景音乐 — ACE-Step 1.5 + Stable Audio 2.5
...
| **Stable Audio** | 2.5 | ~10GB | ~5s | 好 | 不支持 | SA 社区许可 | Spec 指定，备选 |
...
| **BGM 生成备选** | Stable Audio 2.5 | `stabilityai/stable-audio-open-2.5` | ~10GB | Spec 指定方案 |
| **BGM 曲库选择** | 曲库匹配 | — | — | 已有版权曲库直接匹配 |

### ComfyUI 集成
- **ACE-Step**: `ComfyUI-ACE-Step` — 原生内置
- **Stable Audio**: ComfyUI 社区节点支持
...
# Stable Audio 2.5 (备选)
huggingface-cli download stabilityai/stable-audio-open-2.5 \
  --local-dir /data/models/stable-audio/
```
...
| GPU 7 | **ACE-Step 1.5** + Stable Audio + 备用 | ~4+10=14GB | 66GB | N20 BGM + 溢出缓冲 |
...
echo "=== 补充: Stable Audio 2.5 (BGM 备选) ==="
huggingface-cli download stabilityai/stable-audio-open-2.5 \
  --local-dir "$MODEL_ROOT/stable-audio/"
...
| N20: Stable Audio 2.5 (BGM) | ACE-Step 1.5 (升级) + Stable Audio 2.5 (备选) | **已覆盖（升级）** |
